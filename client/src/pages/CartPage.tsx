  ...
  if (!items.length) return (
    <div style={{
      ...
      <p style={{ fontFamily: font, fontSize: "0.85rem", color: "#9BA17B" }}>
        {t("cart.empty.sub", lang)}
      </p>
      <Link href="/products" className="btn-primary">{t("cart.shopnow", lang)}</Link>
    </div>
  );
      ...
      <div className="container" style={{ maxWidth: 1100 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem", flexWrap: "wrap", gap: "0.75rem" }}>
          <h1 style={{ fontFamily: font, fontWeight: 700, fontSize: "1.5rem", color: "#1C201B" }}>
            {t("cart.title", lang)}
            <span style={{ fontFamily: mono, fontSize: "0.65rem", color: "#9BA17B", marginInlineStart: "0.75rem", letterSpacing: "0.15em" }}>
              {items.reduce((s,i)=>s+i.qty,0)} {t("cart.piece", lang)}
            </span>
          </h1>
          <button
            onClick={clear}
          ...
          >
            <Trash2 size={13} /> {t("cart.clearall", lang)}
          </button>
        </div>
              ...
              <div key={item._id} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", flexShrink: 0 }}>
                  ...
                  <button
                    ...
                    style={{ width: 34, height: 34, border: "none", background: "none", cursor: "pointer", color: "#C8BBA4", display: "flex", alignItems: "center", justifyContent: "center", marginInlineStart: 4 }}
                  >
                    <Trash2 size={15} />
                  </button>
                </div>
          ...
          <div className="cart-summary" style={{ background: "#F7F2E8", border: "1px solid rgba(200,187,164,0.35)", padding: "1.5rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem", marginBottom: "1.25rem" }}>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontFamily: font, fontSize: "0.85rem", color: "#9BA17B" }}>{t("cart.subtotal", lang)}</span>
                <span style={{ fontFamily: font, fontSize: "0.85rem", color: "#1C201B" }}>{subtotal.toFixed(2)} {t("common.currency", lang)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontFamily: font, fontSize: "0.85rem", color: "#9BA17B" }}>{t("cart.shipping", lang)}</span>
                <span style={{ fontFamily: font, fontSize: "0.85rem", color: shipping === 0 ? "#059669" : "#1C201B" }}>
                  {shipping === 0 ? t("cart.shipping.free", lang) : `${shipping.toFixed(2)} ${t("common.currency", lang)}`}
                </span>
              </div>
            </div>
            ...
            {subtotal < 200 && (
              <div style={{ background: "rgba(31,57,41,0.06)", border: "1px solid rgba(31,57,41,0.12)", padding: "10px 12px", marginBottom: "1.25rem" }}>
                <p style={{ fontFamily: font, fontSize: "0.75rem", color: "#1F3929", margin: 0 }}>
                  {t("cart.freeship.add", lang)} <strong>{(200 - subtotal).toFixed(2)} {t("common.currency", lang)}</strong> {t("cart.freeship.hint", lang)}
                </p>
            ...
            <div style={{ borderTop: "1px solid rgba(200,187,164,0.4)", paddingTop: "1rem", display: "flex", justifyContent: "space-between", marginBottom: "1.5rem" }}>
              <span style={{ fontFamily: "'Mirza', serif", fontSize: "1.1rem", color: "#1C201B" }}>{t("cart.total", lang)}</span>
              <span style={{ fontFamily: "'Mirza', serif", fontSize: "1.2rem", fontWeight: 500, color: "#1C201B" }}>{total.toFixed(2)} {t("common.currency", lang)}</span>
            </div>
